...
def generate_xs_mtl(X, selector, k, patch_size):
    batch_size = X.size(0)
    with torch.no_grad():
        logits, logsima = selector.forward(X)
    del logsima
    
    if torch.isnan(logits[0]).any():
        print("logits{} is nan".format(0))
    if torch.isnan(logits[1]).any():
        print("logits{} is nan".format(1))
    if torch.isnan(logits[2]).any():
        print("logits{} is nan".format(2))
    
    dims = [logits[0].shape[2], logits[0].shape[3]]
    dims_s = batch_size * dims[0] * dims[1]
    discrete_logits = torch.zeros(dims_s)
    
    for i in range(3):
        logits[i] = logits[i].view(-1, dims_s)
        vals, ind = torch.topk(logits[i], k)
        discrete_logits[ind[0]] += 1
        del vals, ind

    causal_logits = reshape_logits(discrete_logits, batch_size, dims, patch_size)

    X_S = torch.mul(X, causal_logits)
    del X

    return X_S  # , causal_logits


def generate_counterfactuals(X, selector, k, patch_size):
    ...
    dims_s = batch_size * dims[0] * dims[1]
    discrete_logits = torch.zeros(dims_s)
    non_caual_discrete_logits = torch.ones(dims_s)

    # generating noise/variables
    unif_shape = [batch_size, dims[0], dims[1]]
    uniform = (1 - 0) * torch.rand(unif_shape)
    noisy = - torch.log(-torch.log(uniform)).to(device)
    samples_noisy = softmax(noisy)

    for i in range(3):
        logits[i] = logits[i].view(-1, dims_s)
        vals, ind = torch.topk(logits[i], k)
        #causal_logits_task[i][ind[0]] += 1
        discrete_logits[ind[0]] += 1
        del vals, ind

    del logits, unif_shape, uniform, noisy

    discrete_logits_index = torch.nonzero(discrete_logits)

    # print("discrete_logits_index=", discrete_logits_index)
    non_caual_discrete_logits[discrete_logits_index] = 0  

    causal_logits = reshape_logits(discrete_logits, batch_size, dims, patch_size)
    non_causal_logits = reshape_logits(non_caual_discrete_logits, batch_size, dims, patch_size)
    samples_noisy = reshape_logits(samples_noisy, batch_size, dims, patch_size)

    X_Causal = torch.mul(X, causal_logits)
    X_non_Causal = torch.mul(X, non_causal_logits)
    X_CF = X_Causal + torch.mul(non_causal_logits, samples_noisy)

    del X, causal_logits, non_causal_logits, samples_noisy

    return X_CF, X_Causal, X_non_Causal#, causal_logits_task, non_causal_logits_task
...
def Sample_Concrete_for_MTL(Tau_coe, k, logits, train=True):
    ...
    if train == True:
        softmax = nn.Softmax(dim=1)
        logits = logits.view(batch_size, -1).unsqueeze(1)
        
        unif_shape = [batch_size, k, dims]  
        uniform = (1 - 0) * torch.rand(unif_shape)  
        gumbel = - torch.log(-torch.log(uniform + 1e-4) + 1e-4).to(device)  
        ...
        #print("torch.isnan(v)", torch.isnan(v))
        return v
    else:
...
def custom_loss(p_y_xs, p_y_x, batch_size):
    ...
    loss = torch.mean(torch.cosine_similarity(p_y_x, p_y_xs))
    return loss
