...
class SegNet_last_layer_cf(nn.Module):
    def __init__(self, k, patch_size, Tau_coe, device):
        super(SegNet_last_layer_cf, self).__init__()
        # initialise network parameters
        # filter = [32, 64, 128, 256, 256]
        filter = [64, 128, 256, 512, 512]
        self.class_nb = 13
        ...
        self.pred_logits_task = self.out_logits_conv_layer([filter[0], 1])
    ...
    def conv_layer(self, channel, pred=False):
        if not pred:
            conv_block = nn.Sequential(
                nn.Conv2d(in_channels=channel[0], out_channels=channel[1], kernel_size=3, padding=1),
                nn.BatchNorm2d(num_features=channel[1], eps=1e-4, momentum=0.1),
    ...
    def out_logits_conv_layer(self, channel):
        conv_block = nn.Sequential(
            ...
            # nn.BatchNorm2d(num_features=channel[1], eps=1e-4, momentum=0.1),
            # nn.ReLU6(inplace=True),
            nn.Conv2d(in_channels=channel[0], out_channels=channel[1], kernel_size=1),
            nn.MaxPool2d(kernel_size=2, stride=2),
            # nn.BatchNorm2d(num_features=1, eps=1e-4, momentum=0.1),
            # nn.ReLU6(inplace=True),
    ...
    def att_layer(self, channel):
        att_block = nn.Sequential(
            nn.Conv2d(in_channels=channel[0], out_channels=channel[1], kernel_size=1, padding=0),
            nn.BatchNorm2d(channel[1], eps=1e-4, momentum=0.1),
            nn.ReLU6(inplace=True),
            nn.Conv2d(in_channels=channel[1], out_channels=channel[2], kernel_size=1, padding=0),
            nn.BatchNorm2d(channel[2], eps=1e-4, momentum=0.1),
            nn.Sigmoid(),
        )
        return att_block
    ...
    def sample(self, logits):
        # print(logits.size())
        dims = [logits.shape[2], logits.shape[3]]
        dims_s = self.batch_size * dims[0] * dims[1]
        upsample_op = nn.Upsample(scale_factor=self.patch_size, mode='nearest')
        K = self.batch_size * self.k
        ...
        sample_logits = torch.reshape(sample_logits, (self.batch_size, 1, dims[0], dims[1]))
        imp = upsample_op(sample_logits)
        del sample_logits, logits
        return imp
    ...
    def forward(self, x, training=True):
        ...
        for i in range(3):
            ...
            for j in range(5):
                if j == 0:
                    ...
                    if j == 4:
                        att_cf_decoder[i] = (1 - atten_decoder[i][j][1]) * g_decoder[j][-1]

        # t1_logits_pred = self.pred_logits_task(atten_decoder[0][-1][-1])
        # t2_logits_pred = self.pred_logits_task(atten_decoder[1][-1][-1])
        # t3_logits_pred = self.pred_logits_task(atten_decoder[2][-1][-1])

        # t1_causal_logits = self.Sample_gumbel(t1_logits_pred)
        # t2_causal_logits = self.Sample_gumbel(t2_logits_pred)
        # t3_causal_logits = self.Sample_gumbel(t3_logits_pred)

        # t1_non_logis_pred = 1 - t1_logits_pred
        # t2_non_logis_pred = 1 - t2_logits_pred
        # t3_non_logis_pred = 1 - t3_logits_pred

        # t1_cs_fea = torch.mul(atten_decoder[0][-1][-1], t1_causal_logits)
        # t2_cs_fea = torch.mul(atten_decoder[1][-1][-1], t2_causal_logits)
        # t3_cs_fea = torch.mul(atten_decoder[2][-1][-1], t3_causal_logits)

        # t1_non_cs_fea = torch.mul(atten_decoder[0][-1][-1], t1_non_logis_pred)
        # t2_non_cs_fea = torch.mul(atten_decoder[1][-1][-1], t2_non_logis_pred)
        # t3_non_cs_fea = torch.mul(atten_decoder[2][-1][-1], t3_non_logis_pred)

        # define task prediction layers
        # t1_cs_pred = F.log_softmax(self.pred_task1(t1_cs_fea), dim=1)
        # t2_cs_pred = self.pred_task2(t2_cs_fea)
        # t3_cs_pred = self.pred_task3(t3_cs_fea)
        # t3_cs_pred = t3_cs_pred / torch.norm(t3_cs_pred, p=2, dim=1, keepdim=True)

        # define task prediction layers
        # t1_non_cs_pred = F.log_softmax(self.pred_task1(t1_non_cs_fea), dim=1)
        # t2_non_cs_pred = self.pred_task2(t2_non_cs_fea)
        # t3_non_cs_pred = self.pred_task3(t3_non_cs_fea)
        # t3_non_cs_pred = t3_non_cs_pred / torch.norm(t3_non_cs_pred, p=2, dim=1, keepdim=True)
        ...
        t1_cf_pred = F.log_softmax(self.pred_task1(att_cf_decoder[0]), dim=1)
        t2_cf_pred = self.pred_task2(att_cf_decoder[1])
        t3_cf_pred = self.pred_task3(att_cf_decoder[2])
        t3_cf_pred = t3_cf_pred / torch.norm(t3_cf_pred, p=2, dim=1, keepdim=True)

        return [t1_pred, t2_pred, t3_pred], [t1_cf_pred, t2_cf_pred, t3_cf_pred], [atten_decoder[0][-1][-1],
                                                                                   atten_decoder[1][-1][-1],
                                                                                   atten_decoder[2][-1][-1]]
...
class SegNet_Gumbel_Softmax(nn.Module):
    ...
    def forward(self, x, training=True, random_bu=False):
        ...
        # define attention list for tasks
        atten_encoder, atten_decoder = ([0] * 3 for _ in range(2))
        att_cf_decoder, gumbel_logits = ([0] * 3 for _ in range(2))
        sample_gumbel_index = [0] * 3
        for i in range(3):
            atten_encoder[i], atten_decoder[i] = ([0] * 5 for _ in range(2))
                ...
                if j == 0:
                    ...
                    if self.training:
                        if j == 4:
                            ...
                            #sample_gumbel_index[i][0], sample_gumbel_index[i][1] = self.gumbel_sample(gumbel_logits[i])
        ...
        if self.training:
            # task1 and task2
            #gumbel_sample_index_1_2, noise_index_1_2 = self.Union_task_gumbel_index(sample_gumbel_index[0][0],
                                                                                    #sample_gumbel_index[1][0])
            ...
            if self.random_bu:
                ...
                att_cf_decoder_1_2 = self.Add_noise(g_decoder[-1][-1], gumbel_sample_index_1_2, self.noise_factor)

            # task1 and task3
            gumbel_sample_index_1_3 = (sample_gumbel_index[0][0]+ sample_gumbel_index[2][0]) / 2

            if self.random_bu:
                att_cf_decoder_1_3 = self.Add_random_noise(g_decoder[-1][-1], self.noise_factor)
            else:
                att_cf_decoder_1_3 = self.Add_noise(g_decoder[-1][-1], gumbel_sample_index_1_3,
                                                    self.noise_factor)

            # task2 and task3
            gumbel_sample_index_2_3  = (sample_gumbel_index[1][0] + sample_gumbel_index[2][0]) / 2

            if self.random_bu:
                att_cf_decoder_2_3 = self.Add_random_noise(g_decoder[-1][-1], self.noise_factor)
            else:

                att_cf_decoder_2_3 = self.Add_noise(g_decoder[-1][-1], gumbel_sample_index_2_3,self.noise_factor)

            del gumbel_logits, gumbel_sample_index_1_2, gumbel_sample_index_1_3, gumbel_sample_index_2_3
            ...
            t_1_2_to_1_cf_pred = F.log_softmax(self.pred_task1(att_cf_decoder_1_2), dim=1)
            t_1_2_to_2_cf_pred = self.pred_task2(att_cf_decoder_1_2)

            t_1_3_to_1_cf_pred = F.log_softmax(self.pred_task1(att_cf_decoder_1_3), dim=1)
            t_1_3_to_3_cf_pred = self.pred_task3(att_cf_decoder_1_3)
            t_1_3_to_3_cf_pred = t_1_3_to_3_cf_pred / torch.norm(t_1_3_to_3_cf_pred, p=2, dim=1, keepdim=True)

            t_2_3_to_2_cf_pred = self.pred_task2(att_cf_decoder_2_3)
            t_2_3_to_3_cf_pred = self.pred_task3(att_cf_decoder_2_3)
            t_2_3_to_3_cf_pred = t_2_3_to_3_cf_pred / torch.norm(t_2_3_to_3_cf_pred, p=2, dim=1, keepdim=True)

            del att_cf_decoder_1_2, att_cf_decoder_1_3, att_cf_decoder_2_3

            return [t1_pred, t2_pred, t3_pred], [t1_cf_pred, t2_cf_pred, t3_cf_pred], [t_1_2_to_1_cf_pred,
                                                                                       t_1_2_to_2_cf_pred], [
                       t_1_3_to_1_cf_pred, t_1_3_to_3_cf_pred], [t_2_3_to_2_cf_pred, t_2_3_to_3_cf_pred]
